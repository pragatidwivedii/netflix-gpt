...
const Login = () => {
    ...
    return (
        ...
        <div  >
            ...
            {/* background Image */}
            <img src = {BG_IMG} className="h-screen w-full object-cover"/>

            {/* Form */}
            <div className = "  absolute inset-0 flex justify-center items-center ">
            <form className=" bg-black/70 rounded-sm flex flex-col items-center md:items-start py-15 md:py-20 px-10 md:px-15" onSubmit={(e) => {
                e.preventDefault();
            }} >

                <h1 className="mb-5 font-bold text-3xl text-white">{signUp ? 'Sign Up' : 'Sign In'}</h1> 
             
                {(signUp) && (<input type="text" placeholder="Name" className="mb-5 px-5 py-3 w-55 md:w-80 bg-gray-700 placeholder:text-zinc-400 text-white" ref={name} />)}

                <input type="text" placeholder="Email Address" className="mb-5 px-5 py-3 w-55 md:w-80  bg-gray-700 placeholder:text-zinc-400 text-zinc-200" ref={email}/>

                <input type="password" placeholder="Password" className="mb-5 px-5 py-3 w-55 md:w-80  bg-gray-700 placeholder:text-zinc-400 text-zinc-200 " ref={password} />

                <p className="mb-5 text-red-500 font-bold">{errorMsg}</p>
                <button className="mb-5 py-2 w-55 md:w-80 bg-red-700 font-bold text-zinc-200 text-xl cursor-pointer" type = "button" onClick={() => handleValidation(email, password , name , signUp)}>{signUp ? 'Sign Up' : 'Sign In'}</button>
